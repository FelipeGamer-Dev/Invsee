...
export function setInventory(container: FakeDoubleContainer, player: ServerPlayer): void {
  ...
  const inventory: ItemStack[] = player.getInventory().container.getSlots().toArray().reverse();
...
export function getInventorySlots(): number[] {
  let slots: number[] = [];
...
function containerConditional(num: number): number {
